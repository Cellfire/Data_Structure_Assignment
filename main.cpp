...
#define MAX_TOTAL_RECORDS 100000
#define MAX_LIST_LIMIT 100000
...
std::string toLower(const std::string& str) {
    ...
    for (char& ch : lowerStr) ch = std::tolower(static_cast<unsigned char>(ch));
    return lowerStr;
}
...
void searchTransactionType(const std::string& type) const {
    ...
    std::cout << "Searching for payment channel: " << type << "\n";
...
while (temp) {
    if (toLower(temp->data->payment_channel) == searchType) {
        std::cout << std::left
                  << std::setw(12) << temp->data->transaction_id
                  << std::setw(15) << temp->data->payment_channel
...
public:
    ...
    while (i <= j) {
        while (data[i]->location < pivot) i++;
        while (data[j]->location > pivot) j--;
...
void searchTransactionType(const std::string& type) const {
    ...
    bool found = false;
    for (int i = 0; i < size; ++i) {
        if (toLower(data[i]->payment_channel) == searchType) {  
            std::cout << data[i]->transaction_id << " | "
                      << data[i]->payment_channel << " | "
                      << data[i]->location << "\n";
            found = true;
...
void loadCSV(TransactionArray& array, TransactionList& fullList,
    ...
    while (std::getline(file, line)) {
        ...
        if (lineNum % 100000 == 0) std::cout << "[INFO] Processing line " << lineNum << "...\n";
        ...
        try {
            ...
            std::getline(ss, token, ','); t->is_fraud = (token == "1" || token == "true" || token == "True");
            std::getline(ss, t->fraud_type, ',');
            std::getline(ss, token, ','); t->time_since_last_transaction = token.empty() ? 0.0 : std::stod(token);
            std::getline(ss, token, ','); t->spending_deviation_score = token.empty() ? 0.0 : std::stod(token);
            std::getline(ss, token, ','); t->velocity_score = token.empty() ? 0.0 : std::stod(token);
            std::getline(ss, token, ','); t->geo_anomaly_score = token.empty() ? 0.0 : std::stod(token);
...
if (totalLoaded < MAX_TRANSACTIONS) {
    ...
    if (ch == "card") cardList.append(t);
    else if (ch == "ACH") achList.append(t);
    else if (ch == "UPI") upiList.append(t);
    else if (ch == "wire_transfer") wireList.append(t);
}



            ++totalLoaded;
            if (totalLoaded >= MAX_TOTAL_RECORDS) break;
